  ...
  >
    <el-form ref="dataFormRef" :model="state.dataForm" :rules="dataRules" label-width="100" class="mr-4">
      ...
      <el-form-item label="菜单类型" prop="type">
        <el-radio-group v-model="state.dataForm.type" :disabled="!!state.dataForm.id">
        ...
        <el-col :span="12">
          <el-form-item prop="url">
            <template #label>
              <el-tooltip
                effect="dark"
                content="访问的路由地址，如：`system/user/index`，或外网地址则以`http(s)://`开头"
                placement="top"
              >
                <icon icon="ep:question-filled" />
              </el-tooltip>
        ...
        <el-col :span="12">
          ...
          <el-form-item v-if="state.dataForm.type === 'M' || state.dataForm.type === 'B'" prop="authority">
            <template #label>
              <el-tooltip
                effect="dark"
                content="控制器中定义的权限字符，如：@PreAuthorize(`@auth.hasAuthority('system:user:page')`)"
                placement="top"
              >
                <icon icon="ep:question-filled" />
              </el-tooltip>
...
import { getByIdApi, saveOrUpdateApi, postListApi, getCheckAuthorityUniqueApi } from '@/api/system/menu'
...
// 异步加载
const PopoverIcon = defineAsyncComponent(() => import('./popover-icon.vue'))
